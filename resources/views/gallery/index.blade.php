...
@section('content')
    ...
    <div class="max-w-7xl mx-auto mb-12 animate-fade">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end gap-4 mb-10">
            <div>
                <div
                    class="inline-block px-3 py-1 mb-2 rounded-full bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 text-xs font-bold uppercase tracking-wider">
                    Media Center
                </div>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2 group relative h-64 rounded-[2rem] overflow-hidden border border-custom shadow-apple cursor-pointer"
                    ...
                    <div
                        class="absolute bottom-4 left-4 px-3 py-1 bg-black/50 backdrop-blur-md rounded-lg text-white text-xs font-bold border border-white/20">
                        Current Banner
                    </div>
                </div>
                ...
                <div class="group relative h-64 rounded-full overflow-hidden border border-custom shadow-apple cursor-pointer"
                    ...
                    <img src="{{ Auth::user()->profile?->profile_image ? asset('storage/' . Auth::user()->profile->profile_image) : asset('asset/img/profile.svg') }}"
                        ...
                        class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    <div
                        class="absolute bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap px-3 py-1 bg-black/50 backdrop-blur-md rounded-lg text-white text-xs font-bold border border-white/20">
                        Current Avatar
                    </div>
                </div>
            </div>
    ...
    <div id="galleryModal"
        class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/90 backdrop-blur-sm transition-opacity duration-300 opacity-0"
        style="display: none;">

        <button onclick="closeModal()"
            class="absolute top-6 right-6 p-2 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors z-20">
