...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-10 animate-fade">
        ...
        <div class="flex items-center gap-4">
            ...
            <div
                class="relative flex items-center bg-card border border-custom rounded-full shadow-sm group hover:border-primary transition-all duration-300">

                <span
                    class="absolute left-4 text-muted group-hover:text-primary transition-colors pointer-events-none flex items-center">
                    <iconify-icon icon="solar:global-bold-duotone" class="text-lg"></iconify-icon>
                </span>

                <select id="visibility" name="visibility" onchange="this.form.submit()"
                    class="appearance-none bg-transparent pl-11 pr-10 py-2 text-sm font-bold text-text-main outline-none cursor-pointer w-full focus:ring-0">
                    <option value="public" class="bg-card text-text-main" {{ old('visibility', optional(Auth::user()->profile)->visibility) == 'public' ? 'selected' : '' }}>Public</option>
                    <option value="private" class="bg-card text-text-main" {{ old('visibility', optional(Auth::user()->profile)->visibility) == 'private' ? 'selected' : '' }}>Private</option>
                </select>

                <span
                    class="absolute right-4 text-muted pointer-events-none flex items-center group-hover:text-primary transition-colors">
                    <iconify-icon icon="solar:alt-arrow-down-bold" class="text-sm"></iconify-icon>
                </span>
            </div>

            <div
                class="px-5 py-2 rounded-full bg-card border border-custom shadow-sm text-sm font-bold text-text-main flex items-center gap-2">
        ...
        <form action="{{ url()->current() }}" method="GET" class="w-full max-w-2xl">
            <div class="relative group">
                ...
                <div class="absolute inset-y-0 right-2 flex items-center gap-1">
                    {{-- @if(request('search'))
                    <a href="{{ url()->current() }}"
                        class="p-2 text-muted hover:text-red-500 transition-colors flex items-center justify-center"
                        title="Clear Search">
                        <iconify-icon icon="solar:close-circle-bold" class="text-xl"></iconify-icon>
                    </a>
                    @endif --}}
                    <button type="submit"
                        class="px-4 py-1.5 bg-body border border-custom hover:bg-primary hover:text-white hover:border-primary text-text-main rounded-xl text-xs font-bold transition-all shadow-sm">
                        Search
                    </button>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        ...
        <div
            class="group relative bg-card rounded-[2rem] border border-custom p-6 hover:shadow-apple-hover hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">

            <div class="flex justify-between items-start mb-4">
                <div
                    class="w-12 h-12 rounded-2xl bg-body border border-custom flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors duration-300 text-text-main">
                    <iconify-icon icon="solar:code-file-bold-duotone" class="text-2xl"></iconify-icon>
                </div>

                <span
                    class="px-3 py-1 rounded-full bg-gray-100 dark:bg-white/5 text-xs font-bold text-muted border border-custom">
    ...
    @else
    <div
        class="flex flex-col items-center justify-center py-20 bg-card rounded-[2rem] border border-custom shadow-sm text-center">
    ...
    @if($savedRepos->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            @foreach($savedRepos as $repo)
                <div
                    class="group relative bg-card rounded-[2rem] border border-custom p-6 hover:shadow-apple-hover hover:-translate-y-1 transition-all duration-300 flex flex-col h-full overflow-hidden">

                    <div class="flex justify-between items-start mb-4">
                        <div
                            class="w-12 h-12 rounded-2xl bg-body border border-custom flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors duration-300 text-text-main shadow-sm">
                            <iconify-icon icon="solar:code-file-bold-duotone" class="text-2xl"></iconify-icon>
                        </div>

                        @if(isset($repo->private) && $repo->private)
                            <span
                                class="px-3 py-1 rounded-full bg-red-100 dark:bg-red-500/10 text-xs font-bold text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20 flex items-center gap-1.5 shadow-sm">
                                <iconify-icon icon="solar:lock-keyhole-bold-duotone"></iconify-icon> Private
                            </span>
                        @else
                            <span
                                class="px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-xs font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 flex items-center gap-1.5 shadow-sm">
                                <iconify-icon icon="solar:global-bold-duotone"></iconify-icon> Public
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-4 mb-5 text-xs font-bold text-muted">
                        <span class="flex items-center gap-1.5 text-text-main">
                            <iconify-icon icon="solar:code-square-bold-duotone" class="text-primary text-lg"></iconify-icon>
                            {{ $repo->language ?? 'Code' }}
                        </span>

                        <div class="flex items-center gap-3 ml-auto font-medium">
                            <span class="flex items-center gap-1 hover:text-yellow-500 transition-colors" title="Stars">
                                <iconify-icon icon="solar:star-bold-duotone" class="text-lg text-yellow-500/80"></iconify-icon>
                            ...
                            <span class="flex items-center gap-1 hover:text-blue-500 transition-colors" title="Forks">
                                <iconify-icon icon="solar:branching-paths-up-bold-duotone"
                                    class="text-lg text-blue-500/80"></iconify-icon> {{ $repo->forks_count ?? 0 }}
                            </span>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 py-4 border-t border-custom mb-4 mt-auto">
                        <div>
                            <p class="text-[10px] uppercase tracking-wider text-muted font-bold mb-1">Created</p>
                            <div class="flex items-center gap-1.5 text-xs font-medium text-text-main">
                                <iconify-icon icon="solar:calendar-add-linear"></iconify-icon>
